...
2. **Every block has a story and a test, and every primitive has a story.** The story is the catalogue and the a11y panel; the test — Testing Library plus an axe check — is what CI runs. A primitive is storied the day it is installed, not the day someone edits it: one nobody can see is one the next person installs again under another name. Reach for the registry before writing a component at all — [styling.md](../../docs/ai-rules/styling.md#reach-for-shadcn-before-writing-a-component).
