...
/**
 * The panel's read of one of a shop's pages: hidden bands and hidden components included, in the
 * arranged order. The id breaks a tie, so a list read twice is the same list, and the one an add counts in.
 *
 ...
 */
export async function pageOf(db: Prisma.TransactionClient, pageId: string): Promise<Section[]> {
