...
describe("StorefrontProductInfo", () => {
  ...
  it("takes a part's hairline with it when the part is absent", () => {
    const { container } = render(<StorefrontProductInfo shopName="Loja" homeHref="/" name="Blusa" unavailable={false} picker={<p>Sabor</p>} />)

    expect(container.querySelectorAll("[aria-hidden='true']")).toHaveLength(1)
  })

  it("has no 'Sobre este item' without a bulleted list, and no link when the list was all there was", () => {
    const { rerender } = render(<StorefrontProductInfo shopName="Loja" homeHref="/" name="Blusa" unavailable={false} description={"Só um parágrafo.\n\n1. Passo"} />)
    expect(screen.queryByRole("heading", { level: 2 })).not.toBeInTheDocument()

    rerender(<StorefrontProductInfo shopName="Loja" homeHref="/" name="Blusa" unavailable={false} description={"- Mais energia\n- Foco"} />)
    expect(screen.getByRole("heading", { level: 2, name: "Sobre este item" })).toBeInTheDocument()
  ...
  it("has no accessibility violations", async () => {
    const { container } = render(
      <StorefrontProductInfo shopName="Loja" homeHref="/" name="Blusa" unavailable={false} price={<p>R$ 119,90</p>} description={"Intro.\n\n- **Mais energia**"} />,
    )
