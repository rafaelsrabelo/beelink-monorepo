...
2. **Three lanes reach the API, and who is asking picks the lane.** This is the likeliest thing to get wrong here, because the harness template this repo grew from has one lane and every reflex points at it.
   - *The public catalogue, on the server.* A Server Component calls `src/lib/public-api.ts` directly, with `next: { revalidate, tags }` under `store:<slug>` and `catalog:<slug>`. It is a module of its own, apart from `src/lib/api.ts`, because that one pins `cache: "no-store"` — right for a session, wrong for a catalogue served to strangers and to crawlers. `src/lib/revalidate.ts` is the only caller of `revalidateTag`: every admin handler that changes what a visitor is served calls it on a 2xx — a page's draft is served to nobody, so its writes do not, and Publicar does — so an invalidation that escapes leaves a catalogue stale until its window closes rather than stale forever.
   - *Anything the browser asks for.* A BFF route handler under `src/app/api/`, which runs `refuseCrossOrigin()` and forwards `clientIpOf()` from `src/lib/bff.ts` — the API's per-IP rate limit has to see the visitor's address, not this server's. Order creation is the most abusable endpoint in the product, and that limit is its only guard. A handler that reads a shopper's session lives under `src/app/[slug]/api/` instead: those cookies are on the shop's path, and nothing outside it receives them.
...
4. **Session cookies are `bl_access` and `bl_refresh`.** Written by the handlers under `src/app/api/session/`, refreshed in `src/proxy.ts`, `httpOnly` at both ends: page JavaScript never holds a token and web storage holds nothing about a session. Gate: `web/no-web-storage`. Never `hm_*` — that prefix was the harness, and two sessions under one parent domain answering to one name hand each other's tokens around. A shopper's session is `bl_shopper_access` and `bl_shopper_refresh`, on `path=/<slug>` because the account is that shop's alone, written by `src/app/[slug]/api/customer/` and never standing in for the panel's: one person may be both, and the API refuses a token from the wrong door or the wrong shop. `bl_locale`, and the `bl_cart` and `bl_prefs` that arrive with the storefront, carry the prefix for the same reason.
