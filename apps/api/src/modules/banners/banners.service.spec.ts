...
function build(found: { category?: { id: string } | null; product?: { id: string } | null } = {}) {
  ...
  return { service: new BannersService(prisma, stores), create, prisma };
}

describe('BannersService — where a banner points', () => {
  it('keeps the category and clears the other two', async () => {
    const { service, create } = build({ category: { id: 'cat-1' } });

    await service.create('lessari', 'user-1', { ...base, target: 'CATEGORY', categorySlug: 'blusas' });
  ...
  it('refuses a target that names a destination it was not given', async () => {
    ...
    await expect(service.create('lessari', 'user-1', { ...base, target: 'CATEGORY' })).rejects.toMatchObject({
      response: { errorCode: 'BANNER_TARGET_INVALID' },
    });
  });
  ...
  it('refuses a category that is not this shop’s', async () => {
    ...
    await expect(
      service.create('lessari', 'user-1', { ...base, target: 'CATEGORY', categorySlug: 'de-outra-loja' }),
    ).rejects.toMatchObject({ response: { errorCode: 'BANNER_TARGET_INVALID' } });
  ...
  it('lands a new banner last, so it does not rearrange a page already arranged', async () => {
    const { service, create } = build({ category: { id: 'cat-1' } });
  ...
  it('refuses a destination sent without the target that names it', async () => {
    ...
    await expect(
      service.update('lessari', 'user-1', 'banner-1', { categorySlug: 'blusas' }),
    ).rejects.toMatchObject({ response: { errorCode: 'BANNER_TARGET_INVALID' } });
  });
  ...
  it('leaves the destination alone when a patch does not mention it', async () => {
    ...
    await service.update('lessari', 'user-1', 'banner-1', { title: 'Outro título' });

    const data = (prisma.storeBanner.update as unknown as { mock: { calls: [{ data: object }][] } }).mock
      .calls[0][0].data;
